...
Usage:
    python -m DAREG.main --source source.nii.gz --target target.nii.gz --output ./output
    python -m DAREG.main --config config.yaml
"""
...
from .preprocessing import ImageNormalizer, GridManager, ImagePyramid
from .registration import RigidRegistration, AffineRegistration, FFDRegistration
from .postprocessing import ImageTransformer, SegmentationTransformer, QualityMetrics
...
Examples:
  ...
  python -m DAREG.main --source moving.nii.gz --target fixed.nii.gz

  # Full pipeline with all stages
  python -m DAREG.main --source moving.nii.gz --target fixed.nii.gz --method rigid+affine+svffd

  # Use custom config file
  python -m DAREG.main --config registration_config.yaml
        """,
    )
