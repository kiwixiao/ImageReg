...
```bash
# Use specific preset
dareg motion --config-preset svffd_motion_tracking --output auto

# Override preset with CLI args
dareg motion --config-preset svffd_standard --ffd-spacing 8

# Use project-specific config (auto-detected in inputs folder)
dareg motion --json-config inputs_testing/registration_config.yaml
```
