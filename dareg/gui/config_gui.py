...
class ConfigurationGUI(QMainWindow):
    ...
    def _create_input_section(self):
        ...
        self.static_picker.setToolTip(
            "Optional for 4D CT. If omitted, frame 0 is used as reference "
            "and alignment is skipped automatically."
        )
    ...
    def _create_basic_settings_section(self):
        ...
        # Pipeline options
        self.skip_alignment_cb = QCheckBox("Skip alignment (no static image, or already aligned)")
    ...
    def _create_similarity_tab(self) -> QWidget:
        ...
        row += 1
        layout.addWidget(QLabel("ROI Mask (optional):"), row, 0)
        self.roi_mask_picker = FilePickerWidget(
            "", file_filter="NIfTI Files (*.nii *.nii.gz);;All Files (*)"
        )
        layout.addWidget(self.roi_mask_picker, row, 1)
    ...
    def _populate_from_config(self, config: RegistrationConfig):
        ...
        if config.similarity.roi_mask:
            self.roi_mask_picker.setText(config.similarity.roi_mask)
        else:
            self.roi_mask_picker.setText("")
    ...
    def _populate_from_dict(self, config_dict: Dict):
        ...
        if "similarity" in config_dict:
            sim = config_dict["similarity"]
            if "metric" in sim:
                self.similarity_metric_combo.setCurrentText(sim["metric"])
            if "num_bins" in sim:
                self.similarity_bins_spin.setValue(sim["num_bins"])
            if "foreground_threshold" in sim:
                self.similarity_threshold_spin.setValue(sim["foreground_threshold"])
            if "roi_mask" in sim:
                self.roi_mask_picker.setText(sim["roi_mask"])
    ...
    def build_config_dict(self) -> Dict[str, Any]:
        ...
        if self.roi_mask_picker.text():
            config["similarity"]["roi_mask"] = self.roi_mask_picker.text()
