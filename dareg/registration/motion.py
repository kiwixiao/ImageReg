...
from .svffd import SVFFDRegistration
...
class MotionRegistration:
    ...
    def __init__(
        ...
        Args:
            ...
            roi_mask: Optional binary ROI mask to restrict registration computation region.
                      If provided, similarity metrics are computed only within this mask.
                      Useful for focusing registration on specific anatomical structures.
        """
        self.device = torch.device(device)
        self.config = config
        self.registration_model = registration_model
        self.roi_mask = roi_mask  # Store ROI mask for all registration stages
        ...
        if roi_mask is not None:
            logger.info(f"  ROI mask: enabled ({roi_mask.sum().item():.0f} voxels)")
        ...
        with Timer("Alignment registration"):
            ...
            if "rigid" in stages:
                ...
                if self.config and hasattr(self.config, "similarity"):
                    ...
                    if hasattr(sc, "num_samples") and sc.num_samples is not None:
                        rigid_kwargs["num_samples"] = sc.num_samples
                    if hasattr(sc, "sample_ratio") and sc.sample_ratio is not None:
                        rigid_kwargs["sample_ratio"] = sc.sample_ratio
                # Pass ROI mask if available
                if self.roi_mask is not None:
                    rigid_kwargs["roi_mask"] = self.roi_mask
                logger.debug(f"  RIGID CONFIG: {rigid_kwargs}")

                rigid_reg = RigidRegistration(**rigid_kwargs)
                rigid_result = rigid_reg.register(current_source, target_image, progress_tracker=progress_tracker)
            ...
            if "affine" in stages:
                ...
                if self.config and hasattr(self.config, "similarity"):
                    ...
                    if hasattr(sc, "num_samples") and sc.num_samples is not None:
                        affine_kwargs["num_samples"] = sc.num_samples
                    if hasattr(sc, "sample_ratio") and sc.sample_ratio is not None:
                        affine_kwargs["sample_ratio"] = sc.sample_ratio
                # Pass ROI mask if available
                if self.roi_mask is not None:
                    affine_kwargs["roi_mask"] = self.roi_mask
            ...
            if "ffd" in stages or "svffd" in stages:
                ...
                # Extract config - use svffd section if available and model is svffd
                ffd_kwargs = {"device": self.device}
                config_section = "svffd" if ffd_type == "svffd" and self.config and hasattr(self.config, "svffd") else "ffd"
                if self.config and hasattr(self.config, config_section):
                    fc = getattr(self.config, config_section)
                    ffd_kwargs.update({
                    ...
                    if hasattr(fc, "regularization"):
                        ...
                        if ffd_type == "svffd":
                            if hasattr(fc.regularization, "velocity_smoothing_sigma"):
                                ffd_kwargs["velocity_smoothing_sigma"] = fc.regularization.velocity_smoothing_sigma
                            if hasattr(fc.regularization, "laplacian_weight"):
                                ffd_kwargs["laplacian_weight"] = fc.regularization.laplacian_weight
                            if hasattr(fc.regularization, "jacobian_penalty"):
                                ffd_kwargs["jacobian_penalty"] = fc.regularization.jacobian_penalty
                    if hasattr(fc, "convergence"):
                        # FFD uses epsilon/delta (MIRTK-style), SVFFD uses convergence_delta/patience
                        if ffd_type == "svffd":
                            ffd_kwargs["convergence_delta"] = fc.convergence.min_delta
                            ffd_kwargs["convergence_patience"] = fc.convergence.patience
                        else:
                            ffd_kwargs["epsilon"] = fc.convergence.min_delta
                            # FFD doesn't use patience (uses max_rejected_streak instead)
                    # SVFFD integration steps
                    if ffd_type == "svffd" and hasattr(fc, "integration_steps"):
                        ffd_kwargs["integration_steps"] = fc.integration_steps
                if self.config and hasattr(self.config, "similarity"):
                    sc = self.config.similarity
                    ffd_kwargs["num_bins"] = sc.num_bins
                    ffd_kwargs["foreground_threshold"] = sc.foreground_threshold
                    if hasattr(sc, "num_samples") and sc.num_samples is not None:
                        ffd_kwargs["num_samples"] = sc.num_samples
                    if hasattr(sc, "sample_ratio") and sc.sample_ratio is not None:
                        ffd_kwargs["sample_ratio"] = sc.sample_ratio
                # Pass ROI mask if available
                if self.roi_mask is not None:
                    ffd_kwargs["roi_mask"] = self.roi_mask
                logger.debug(f"  {ffd_type.upper()} CONFIG: {ffd_kwargs}")

                # Select correct registration class
                if ffd_type == "svffd":
                    ffd_reg = SVFFDRegistration(**ffd_kwargs)
                else:
                ...
                intermediate_results[ffd_type] = current_source  # Store for visualization
                intermediate_transforms[ffd_type] = ffd_result.transform
                if hasattr(ffd_result, 'convergence_data'):
                    convergence_data[ffd_type] = ffd_result.convergence_data
                elif hasattr(ffd_result, 'loss_history'):
                    convergence_data[ffd_type] = ffd_result.loss_history
            ...
            if output_dir is not None:
                try:
                    ...
                    save_alignment_progression(
                        ...
                        ffd_result=intermediate_results.get(ffd_type),
                        output_dir=vis_dir,
                        prefix="alignment",
                        ...
                        ffd_transform=intermediate_transforms.get(ffd_type),
                        ffd_label=ffd_type.upper(),
                        show_grid=True,
                    )
                ...
                # Create grid+arrows multi-slice visualizations for each transform stage
                try:
                    from ..visualization.alignment_overlay import create_multi_slice_grid_with_arrows

                    debug_dir = Path(output_dir) / "alignment" / "debug"
                    debug_dir.mkdir(parents=True, exist_ok=True)

                    # Use frame0 as anatomical background
                    anat_np = frame0_resampled.tensor().squeeze().cpu().numpy()

                    transforms_to_viz = [
                        ('rigid', intermediate_transforms.get('rigid')),
                        ('affine', intermediate_transforms.get('affine')),
                        (ffd_type, intermediate_transforms.get(ffd_type)),
                    ]

                    for stage_name, transform in transforms_to_viz:
                        if transform is not None:
                            logger.info(f"Creating grid+arrows visualization for {stage_name}...")
                            try:
                                output_base = debug_dir / f"grid_arrows_{stage_name}"
                                create_multi_slice_grid_with_arrows(
                                    grid=common_grid,
                                    transform=transform,
                                    anatomical_image=anat_np,
                                    output_path=output_base,
                                    target_slices=6,  # Fewer slices for clearer view
                                    stage_name=stage_name.upper(),
                                    arrow_step=24,  # Every 2nd grid point (sparser arrows)
                                    grid_stride=12,
                                    grid_alpha=0.4,  # Slightly less grid opacity
                                )
                                logger.info(f"  Saved grid+arrows for {stage_name} to {debug_dir}")
                            except Exception as e:
                                logger.warning(f"Could not create grid+arrows for {stage_name}: {e}")
                                import traceback
                                traceback.print_exc()

                except ImportError as e:
                    logger.warning(f"Could not import grid+arrows visualization: {e}")
                except Exception as e:
                    logger.warning(f"Could not create grid+arrows visualizations: {e}")
            ...
            self._alignment_intermediates = AlignmentIntermediates(
                ...
                ffd_warped=intermediate_results.get(ffd_type),
                rigid_transform=intermediate_transforms.get('rigid'),
                affine_transform=intermediate_transforms.get('affine'),
                ffd_transform=intermediate_transforms.get(ffd_type),
    ...
    def refine_longitudinal(
        ...
        for i, composed in enumerate(longitudinal_transforms):
            ...
            with Timer(f"Refine 0→{src_idx}"):
                ...
                ffd_type = "svffd" if "svffd" in self.registration_model else "ffd"
                ffd_kwargs = {"device": self.device, "pyramid_levels": 1}
                config_section = "svffd" if ffd_type == "svffd" and self.config and hasattr(self.config, "svffd") else "ffd"
                if self.config and hasattr(self.config, config_section):
                    fc = getattr(self.config, config_section)
                    ffd_kwargs.update({
    ...
    def propagate_segmentation(
        ...
        for composed in longitudinal_transforms:
            ...
            with Timer(f"Propagate to frame {composed.source_idx}"):
                ...
                try:
                    # CRITICAL: update_buffers=True ensures SVFFD recomputes displacement field
                    # for the inverse transform. Without this, the inverse may use stale buffers.
                    inverse_transform = transform.inverse(update_buffers=True)
    ...
    def _save_frames(self, image_4d: Image4D, output_dir: Path):
        ...
        Naming convention: dynamic_extracted_f###_t###.nii.gz
        - f### = absolute frame index from original 4D (1-indexed)
        - t### = relative frame index within extracted set (1-indexed)
        """
        ...
        for i in range(image_4d.num_frames):
            ...
            # New naming: f### = absolute (1-indexed), t### = relative (1-indexed)
            # i is 0-indexed relative, image_4d.start_frame is 0-indexed absolute start
            abs_idx = image_4d.start_frame + i + 1  # 1-indexed absolute
            rel_idx = i + 1  # 1-indexed relative

            # Use nibabel-based save_nifti for correct affine preservation
            save_nifti(
                frame_nifti,
                frames_dir / f"dynamic_extracted_f{abs_idx:03d}_t{rel_idx:03d}.nii.gz",
                affine,
                description=f"dynamic extracted frame abs={abs_idx} rel={rel_idx}"
            )

        logger.info(f"Saved {image_4d.num_frames} dynamic extracted frames to {frames_dir}")
    ...
    def _save_segmentation(self, seg: Image, frame_idx: int, image_4d: Image4D, output_dir: Path):
        ...
        Naming convention: seg_f###_t###.nii.gz
        - f### = absolute frame index from original 4D (1-indexed)
        - t### = relative frame index within extracted set (1-indexed)
        """
        ...
        # New naming: f### = absolute (1-indexed), t### = relative (1-indexed)
        abs_idx = image_4d.start_frame + frame_idx + 1  # 1-indexed absolute
        rel_idx = frame_idx + 1  # 1-indexed relative

        # Log for debugging - show actual segmentation resolution
        grid = seg.grid()
        logger.info(f"Saving seg_f{abs_idx:03d}_t{rel_idx:03d} (high-res):")
        ...
        save_segmentation(
            seg_nifti,
            seg_dir / f"seg_f{abs_idx:03d}_t{rel_idx:03d}.nii.gz",
            affine,
            description=f"segmentation frame abs={abs_idx} rel={rel_idx} (high-res)"
        )
        logger.info(f"  Saved: seg_f{abs_idx:03d}_t{rel_idx:03d}.nii.gz")
    ...
    def _save_segmentation_frame_grid(self, seg: Image, frame_idx: int, image_4d: Image4D, output_dir: Path):
        ...
        Naming convention: seg_f###_t###_frame_grid.nii.gz
        - f### = absolute frame index from original 4D (1-indexed)
        - t### = relative frame index within extracted set (1-indexed)

        Args:
            seg: Segmentation image (already resampled to frame grid)
            frame_idx: Frame index (relative, 0-indexed)
        ...
        save_segmentation(
            seg_nifti,
            seg_dir / f"seg_f{abs_idx:03d}_t{rel_idx:03d}_frame_grid.nii.gz",
            affine,
            description=f"segmentation frame abs={abs_idx} rel={rel_idx} (frame grid)"
        )
        logger.debug(f"Saved frame-grid segmentation for frame f{abs_idx:03d}_t{rel_idx:03d}")

    def _resample_segmentation_to_grid(self, segmentation: Image, output_grid: Grid) -> Image:
        """Resample segmentation to a different grid (pure resampling, no transform).

        This is used for frame 0 where no transform is needed - just resample the
        segmentation from its native high-res grid to the frame's lower-res grid.

        CRITICAL: Uses nearest neighbor interpolation to preserve discrete labels.

        Args:
            segmentation: Source segmentation image
            output_grid: Target grid to resample to

        Returns:
            Resampled segmentation as deepali Image
        """
        import torch.nn.functional as F

        # Get segmentation tensor
        seg_tensor = segmentation.tensor()
        if seg_tensor.dim() == 3:
            seg_tensor = seg_tensor.unsqueeze(0).unsqueeze(0)  # [1, 1, D, H, W]
        elif seg_tensor.dim() == 4:
            seg_tensor = seg_tensor.unsqueeze(0)  # [1, 1, D, H, W]

        seg_grid = segmentation.grid()

        # Get output grid coordinates in world space, then convert to segmentation's cube space
        # This is the correct way to resample: sample the source at locations defined by target grid
        output_cube_coords = output_grid.coords(device=seg_tensor.device, dtype=torch.float32)
        if output_cube_coords.dim() == 4:
            output_cube_coords = output_cube_coords.unsqueeze(0)  # [1, D, H, W, 3]

        # Convert output cube coords → world coords → segmentation cube coords
        output_world_coords = output_grid.cube_to_world(output_cube_coords)
        seg_cube_coords = seg_grid.world_to_cube(output_world_coords)

        # Sample segmentation at these coordinates using nearest neighbor
        ...
        # Return as deepali Image with output grid
        return Image(resampled.squeeze(0), grid=output_grid)

    def _save_static_to_all_frames(
        self,
        static_image: Image,
        alignment_transform: SpatialTransform,
        longitudinal_transforms: List[ComposedTransform],
        common_grid: Grid,
        output_dir: Path,
    ):
        """
        Save static image moved to all frames at original resolution.

        This creates static_frame_000.nii.gz, static_frame_001.nii.gz, etc.
        where each file is the static image transformed to that frame's space
        while preserving the static image's original resolution.

        Args:
            static_image: Original static high-res image
            alignment_transform: Static → Frame0 transform
            longitudinal_transforms: List of Frame_i → Frame0 transforms
            common_grid: Common grid where transforms were learned
            output_dir: Output directory
        """
        ...
        logger.info("=" * 60)
        logger.info("SAVING STATIC IMAGE TO ALL FRAMES (original resolution)")
        logger.info("=" * 60)

        # Frame 0: Just apply alignment transform
        logger.info("Saving static → frame 0 (original resolution)...")
        aligned_static_0 = apply_transform_preserve_resolution(
            source_image=static_image,
            transform=alignment_transform,
        ...
        # Save frame 0
        self._save_static_frame(aligned_static_0, 0, static_image.grid(), final_dir)

        # Other frames: Apply alignment then inverse of longitudinal transform
        for composed in longitudinal_transforms:
            frame_idx = composed.source_idx

            # Skip frame 0 (already saved above)
            if frame_idx == 0:
                continue

            logger.info(f"Saving static → frame {frame_idx} (original resolution)...")

            # longitudinal_transform maps frame_i → frame_0
            # We need frame_0 → frame_i, so use inverse
            try:
                # CRITICAL: update_buffers=True ensures SVFFD recomputes displacement field
                inverse_longitudinal = composed.transform.inverse(update_buffers=True)
                logger.debug(f"  Frame {frame_idx}: Using analytical inverse")
            except (NotImplementedError, Exception) as e:
                logger.warning(f"  Frame {frame_idx}: Transform doesn't support inverse ({e}), using approximate")
                from ..postprocessing.segmentation import approximate_ffd_inverse
                # ApproximateInverseTransform now inherits from NonRigidTransform
                # so it works with all deepali composition and transform utilities
                inverse_longitudinal = approximate_ffd_inverse(composed.transform, max_iterations=10, tolerance=1e-5)

            # Apply transforms SEQUENTIALLY (alignment then inverse longitudinal)
            # Step 1: Static → Frame0 (alignment)
            aligned_static_0_for_frame = apply_transform_preserve_resolution(
                source_image=static_image,
                transform=alignment_transform,
                target_grid=common_grid,
                sampling="linear",
            ...
            # Step 2: Frame0 → Frame_i (inverse longitudinal)
            # Both analytical inverse and ApproximateInverseTransform are SpatialTransforms
            # so we can use the same apply_transform_preserve_resolution for both
            aligned_static_i = apply_transform_preserve_resolution(
                source_image=aligned_static_0_for_frame,
                transform=inverse_longitudinal,
                target_grid=common_grid,
                sampling="linear",
            )

            # Save this frame
            self._save_static_frame(aligned_static_i, frame_idx, static_image.grid(), final_dir)

        logger.info(f"Saved static image to {len(longitudinal_transforms) + 1} frames in {final_dir}")

    def _save_static_frame(self, image: Image, frame_idx: int, original_grid: Grid, output_dir: Path):
        """Save a single static frame with proper affine from original grid."""
        img_np = image.tensor().cpu().numpy()
        if img_np.ndim == 4:
            img_np = img_np.squeeze(0)

        # Transpose from torch [D, H, W] to NIfTI [X, Y, Z] order
        img_nifti = np.transpose(img_np, (2, 1, 0))

        # Use the IMAGE's actual grid for affine construction (preserves original resolution)
        affine = _construct_affine_from_grid(image.grid())
        affine[:2] *= -1  # LPS to RAS

        save_nifti(
            img_nifti,
            output_dir / f"static_image_moved_to_frame_{frame_idx:03d}.nii.gz",
            affine,
            description=f"static image moved to frame {frame_idx} (original resolution)"
        )
        logger.info(f"  Saved: static_image_moved_to_frame_{frame_idx:03d}.nii.gz")
    ...
    def run_full_pipeline(
        ...
        if static_image is not None and not skip_alignment:
            total_phases += 1  # alignment
        total_phases += 1  # pairwise
        total_phases += 1  # compose
        if not skip_refinement:
            total_phases += 1  # refine
        if segmentation is not None:
            total_phases += 1  # propagate

        # Initialize progress tracker if provided
        if progress_tracker:
            progress_tracker.start(total_phases=total_phases, max_frames=image_4d.num_frames)
        ...
        with Timer("Total pipeline") as total_timer:
            ...
            if static_image is not None and not skip_alignment:
                if progress_tracker:
                    progress_tracker.set_phase("alignment", phase_index)
                ...
                result.alignment_intermediates = self._alignment_intermediates  # CRITICAL: populate for save_results
                seg_0 = aligned_seg
            ...
            if progress_tracker:
                progress_tracker.set_phase("pairwise", phase_index)
            pairwise_results = self._register_pairwise_incremental(
                image_4d, seg_0, output_dir, progress_tracker=progress_tracker
            )
            ...
            if progress_tracker:
                progress_tracker.set_phase("compose", phase_index)
            ...
            if not skip_refinement:
                if progress_tracker:
                    progress_tracker.set_phase("refine", phase_index)
            ...
            if seg_0 is not None:
                if progress_tracker:
                    progress_tracker.set_phase("propagate", phase_index)
        ...
        # Signal completion
        if progress_tracker:
            progress_tracker.finish(success=True, message="Pipeline completed successfully")
    ...
    def _register_pairwise_incremental(
        ...
        for i, (src_idx, tgt_idx, source, target) in enumerate(pairs):
            ...
            with Timer(f"Pairwise {src_idx}→{tgt_idx}"):
                ...
                ffd_result = ffd_reg.register(source_norm, target_norm, progress_tracker=progress_tracker)
    ...
    def _propagate_segmentation_incremental(
        ...
        # FRAME 0 SPECIAL CASE: Save segmentation for reference frame (no transform needed)
        # Frame 0 is the reference, so segmentation_0 IS the frame 0 segmentation
        if output_dir is not None:
            logger.info("Saving frame 0 segmentation (reference frame)...")

            # Save high-res version (original resolution) for frame 0
            self._save_segmentation(segmentation_0, 0, image_4d, output_dir)

            # Save frame-grid version for frame 0 (resampled to match frame_000.nii.gz)
            # This is pure resampling without transform (identity)
            frame0_grid = image_4d.get_frame(0).grid()
            seg_0_frame_grid = self._resample_segmentation_to_grid(segmentation_0, frame0_grid)
            self._save_segmentation_frame_grid(seg_0_frame_grid, 0, image_4d, output_dir)

            # Log with computed names for frame 0
            abs_idx_0 = image_4d.start_frame + 0 + 1  # 1-indexed absolute
            logger.info(f"  Saved: seg_f{abs_idx_0:03d}_t001.nii.gz and seg_f{abs_idx_0:03d}_t001_frame_grid.nii.gz")
    ...
    def save_results(
        ...
        - final_results/: Static image moved to all frames at original resolution
                         (static_frame_000.nii.gz, static_frame_001.nii.gz, etc.)
        ...
        # Create directory structure (cleaned up - no empty segmentation_results)
        ...
        if hasattr(self, '_alignment_intermediates') and self._alignment_intermediates is not None:
            ...
            if ai.ffd_transform is not None:
                ...
                self.composer.save_transform(
                    ai.ffd_transform,
                    transforms_dir / f"{ffd_type}_transform.pth",
                    metadata={"type": ffd_type, "stage": "alignment"},
                )
            ...
            # Target (frame 0) reference - for overlay verification
            if ai.target_image is not None:
                self._save_image_nifti(
                    ai.target_image,
                    final_dir / "frame0_reference.nii.gz",
                    ai.target_image,
                )
        ...
        # ================================================================
        # SAVE STATIC IMAGE TO ALL FRAMES (original resolution)
        # ================================================================
        # This saves static_frame_000.nii.gz, static_frame_001.nii.gz, etc.
        # where each file is the static image transformed to that frame's space
        # while preserving the static image's original resolution.

        # DEBUG: Check each condition
        logger.debug(f"SAVE_STATIC_TO_ALL_FRAMES condition check:")
        logger.debug(f"  static_image is not None: {static_image is not None}")
        logger.debug(f"  result.alignment_transform is not None: {result.alignment_transform is not None}")
        logger.debug(f"  result.longitudinal_transforms (truthy): {bool(result.longitudinal_transforms)}")
        logger.debug(f"  result.longitudinal_transforms count: {len(result.longitudinal_transforms) if result.longitudinal_transforms else 0}")
        logger.debug(f"  result.alignment_intermediates is not None: {result.alignment_intermediates is not None}")
        if result.alignment_intermediates is not None:
            logger.debug(f"  result.alignment_intermediates.common_grid is not None: {result.alignment_intermediates.common_grid is not None}")
        else:
            logger.debug(f"  result.alignment_intermediates.common_grid: N/A (intermediates is None)")

        if (static_image is not None and
            result.alignment_transform is not None and
            result.longitudinal_transforms and
            result.alignment_intermediates is not None and
            result.alignment_intermediates.common_grid is not None):

            self._save_static_to_all_frames(
                static_image=static_image,
                alignment_transform=result.alignment_transform,
                longitudinal_transforms=result.longitudinal_transforms,
                common_grid=result.alignment_intermediates.common_grid,
        ...
        # during run_full_pipeline() as seg_f###_t###.nii.gz (f=absolute, t=relative).
    ...
    def _create_alignment_visualizations(self, ai: AlignmentIntermediates, debug_dir: Path):
        ...
        try:
            ...
            if ai.source_image is not None:
                ...
                # Per-stage results
                ffd_label = "svffd" if "svffd" in self.registration_model else "ffd"
                for stage, warped in [('rigid', ai.rigid_warped),
                                       ('affine', ai.affine_warped),
                                       (ffd_label, ai.ffd_warped)]:
            ...
            # Note: Grid+arrows visualizations are now generated immediately after alignment
            # in the align_static_to_frame0() method, not here in save_results()
